...
createUIStopButton :: proc(page: ^app_framework.Page, fe: ^fire_engine.FireEngine) -> ^app_framework.Element {
    stop_button := app_framework.createElement("stop_button", nil)
    stop_button.onDraw = proc(el: ^app_framework.Element, ctx: ^vg.Context, user_data: rawptr) {
        ...
        draw_stop_icon(ctx, center_x, center_y, icon_size, icon_size, color)
    }

    page->addConnection(stop_button.onPressed, proc(value: any, user_data: rawptr) {
        pcd := cast(^app_framework.PageConnectionData)user_data
        fe := cast(^fire_engine.FireEngine)pcd.user_data
        fe.transport->stop()
    }, fe)
    page->addChild(stop_button)
    return stop_button
}
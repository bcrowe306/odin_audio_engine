...
initializeControl :: proc(ptr: rawptr, control_surface: ^ControlSurface, device_name: string, fe: ^FireEngine) {
...
control_sendMidi :: proc(control_ptr: rawptr, msg: ShortMessage) {
    ...
    if control.fe != nil && control.fe.midi_engine != nil {
        control.fe.midi_engine.sendMsg(control.fe.midi_engine, control.device_name, msg)
    }
}
...
control_sendSysex :: proc(control_ptr: rawptr, msg: []u8) {
    control := cast(^Control)control_ptr
    if control.fe != nil && control.fe.midi_engine != nil {
        control.fe.midi_engine.sendSysexMsg(control.fe.midi_engine, control.device_name, msg)
    }
}